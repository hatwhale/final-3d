...
namespace rendering {
	...
	private: System::Windows::Forms::CheckBox^  transparencyÑheckBox;
	...
	public: 
		...
		RenderForm(void)
		{
			...
			this->transparencyÑheckBox->Click += gcnew System::EventHandler(this, &RenderForm::Render);
...
#pragma region Windows Form Designer generated code
		void InitializeComponent(void)
		{
			...
			this->transparencyÑheckBox = (gcnew System::Windows::Forms::CheckBox());
			...
			this->carouselTimer->Tick += gcnew System::EventHandler(this, &RenderForm::Ñarousel);
			// 
			// popButton
			// 
			...
			// 
			// transparencyÑheckBox
			// 
			this->transparencyÑheckBox->AutoSize = true;
			this->transparencyÑheckBox->Location = System::Drawing::Point(683, 31);
			this->transparencyÑheckBox->Name = L"transparencyÑheckBox";
			this->transparencyÑheckBox->Size = System::Drawing::Size(98, 17);
			this->transparencyÑheckBox->TabIndex = 11;
			this->transparencyÑheckBox->Text = L"early depth test";
			this->transparencyÑheckBox->UseVisualStyleBackColor = true;
			this->transparencyÑheckBox->Click += gcnew System::EventHandler(this, &RenderForm::EarlyDepthTestSwitch);
			// 
			// pegTopCheckBox
			// 
			...
			this->Controls->Add(this->transparencyÑheckBox);
...
#pragma endregion
	private: System::Void InfoMessage(System::Object^  sender, System::EventArgs^  e) {
				 String^ message = "Áîëåå ïîäðîáíûå êîìåíòàðèè - â çàãîëîâî÷íûõ ôàéëàõ ïðîåêòà.\n\n" +
								   "* ComboBox â ëåâîì âåðõíåì óãëó: çàãðóçêà îáúåêòà èç òåêñòîâîãî ôàéëà èëè ïðîöåäóðíîå ïîñòðîåíèå ìîäåëè;\n" +
								   "* ComboBox â ïðàâîì âåðõíåì óãëó: âûáîð òåêñòóðû äëÿ ïîñëåäíåãî äîáàâëåííîãî îáúåêòà (äîñòóïíà çàãðóçêà èç ôàéëà);\n\n" + 
								   "* Button pop: óäàëåíèå ïîñëåäíåãî äîáàâëåííîãî îáúåêòà;\n" +
								   "* Button frame save: ñîõðàíåíèå òåêóùåãî êàäðà â data/frames;\n" +
								   "* Button model save: ñîõðàíåíèå ïîñëåäíåãî äîáàâëåííîãî îáúåêòà è åãî õàðàêòåðèñòèê â òåêñòîâûé ôàéë â data/models/saved;\n\n" +
								   "* Buttons translate, scale, rotate: àêòèâàöèÿ ðàçëè÷íûõ ðåæèìîâ ïåðåìåùåíèÿ îáúåêòîâ èëè èñòî÷íèêà îñâåùåíèÿ:\n" +
								   " - ïðè çàæàòîé ëåâîé êíîïêå ìûøè îñóùåñòâëÿþòñÿ ñäâèãè, ìàñøòàáèðîâàíèÿ âäîëü è âðàùåíèÿ âîêðóã îñåé x è y;\n " +
								   " - ïðè çàæàòîé ïðàâîé êíîïêå ìûøè - âäîëü è âîêðóã îñè z;\n" + 
								   " - êîë¸ñèêî ìûøè ïîçâîëÿåò ìåíÿòü ìàñøòàá ïîñëåäíåãî äîáàâëåííîãî îáúåêòà ïî âñåì òð¸ì îñÿì îäíîâðåìåííî;\n" + 
								   "* Button parallel/orthogonal projection: âûáîð òèïà ïðîåêöèè (ïàðàëëåëüíàÿ èëè îäíîòî÷å÷íàÿ);\n" +
								   "* CheckBoxes peg-top, carousel: ðåæèìû âîë÷êà è êàðóñåëè ñîîòâåòñòâåííî;\n\n" +
								   "* CheckBox flood fill: çàëèâêà ãðàíåé îáúåêòîâ (èíà÷å îòîáðàæàþòñÿ êàðêàñíûå ìîäåëè);\n" +
								   "* CheckBox early depth test: óäàëåíèå íåâèäèìûõ ðåáåð è ãðàíåé ïî àëãîðèòìó Ðîáåðòñà äëÿ êàæäîé ìîäåëè;\n" +
								   "* CheckBox depth test: óäàëåíèå íåâèäèìûõ ðåáåð è ãðàíåé ïðè ïîìîùè áóôåðà ãëóáèíû;\n" +
								   "* CheckBox shadow test: îòîáðàæåíèå òåíåé ïðè ïîìîùè shadow map - áóôåðà ïñåâäî-ãëóáèíû;\n" +
								   "* ComboBox shading: âûáîð ðåæèìà çàòåíåíèÿ îáúåêòîâ (ïëîñêîå, ïî Ãóðî, ïî Ôîíãó).\n\n" +
								   "* Button active/deactivate light: âêëþ÷åíèå/âûêëþ÷åíèå èñòî÷íèêà îñâåùåíèÿ;\n" +
								   "* Button move light/objects: âûáîð ìåæäó ïåðåìåùåíèåì îáúåêòîâ è ïåðåìåùåíèåì èñòî÷íèêà îñâåùåíèÿ;\n" +
								   "* Button frame/depth buffer: âûáîð ìåæäó áóôåðàìè êàäðà è ãëóáèíû;\n\n" +
								   "Ïðè ïîñòðîåíèè òåíåé è ïåðåìåùåíèè èñòî÷íèêà ñâåòà ñöåíà îòîáðàæàåòñÿ ñ êàìåðîé â èñòî÷íèêå.\n" +
								   "Äîñòóïíû èçìåíåíèÿ ïàðàìåòðîâ èñòî÷íèêà îñâåùåíèÿ è ñâîéñòâ ìàòåðèàëà ïîñëåäíåãî äîáàâëåííîãî îáúåêòà.";
				 String^ caption = "ÊÄÇ 1 ïî Êîìïüþòåðíîé ãðàôèêå, Êàñüÿíîâ È. À., 2017";
	...
	private: System::Void Ñarousel(System::Object^  sender, System::EventArgs^  e) {
				 Vector^ angles = gcnew Vector(1, -1, -1);
				 angles *= this->event_data->speed * System::Math::PI / 180;
